...
export function attachEventLogging(player, log, { includeProgress = false } = {}) {
  ...
  // so the log shows the values, not just the name. The session stamps are
  // left out; the error object prints as its code name, code and message.
  const fmt = (v) => v && typeof v === "object"
    ? (v.codeName ? `${v.codeName}(${v.code}) ${v.message ?? ""}`.trim() : JSON.stringify(v))
    : String(v);
  player.addAnalyticsListener((e) => {
    const parts = Object.entries(e)
      .filter(([k, v]) => k !== "type" && k !== "sessionId" && k !== "assetUrl" && v !== undefined)
      .map(([k, v]) => `${k}=${fmt(v)}`);
